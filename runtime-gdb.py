...
def is_iface_no_struct(val):
	try:
		return str(val['tab'].type) == "runtime.itab *" and str(val['data'].type) == "void *"
	except gdb.error:
		pass


def is_eface_no_struct(val):
	try:
		return str(val['_type'].type) == "runtime._type *" and str(val['data'].type) == "void *"
	except gdb.error:
		pass
...
def iface_commontype(obj):
	if is_iface_no_struct(obj):
		go_type_ptr = obj['tab']['_type']
	elif is_eface_no_struct(obj):
...
def iface_dtype(obj):
	...
	if dynamic_gdb_type is None:
		dynamic_gdb_type = lookup_type(full_dwarf_name(dynamic_go_type))
		if dynamic_go_type is None:
		    return
...
def string_to_python_string(obj):
	l = int(obj['len'])
	return obj['str'].string("utf-8", "ignore", l)


def get_pkgName_by_tfn(tfn):
	pathWithFun = tfn.split('<')[1].split('>')[0]
	return "%s.%s" % (pathWithFun.split('.')[0], pathWithFun.split('.')[1].replace('(',
		'').replace('*', '').replace(')', ''))
	pass


def full_dwarf_name(dynamic_go_type):
	uncommonType = dynamic_go_type['*reflect.uncommonType']
	if uncommonType != None:
		try: 
			pkgPath = uncommonType.dereference()['pkgPath']
			if pkgPath != None:
				pkgName = pkgPath.dereference()
				shortName = uncommonType.dereference()['name'].dereference()
				fullName = "%s.%s" % (string_to_python_string(pkgName), string_to_python_string(shortName))

				return fullName

		except Exception:
			#get fullPath by fname
			for ptr in SliceValue(uncommonType.dereference()['methods']):
				tfn = ptr['tfn']
				break
			fullName = get_pkgName_by_tfn(str(tfn))
			return fullName
